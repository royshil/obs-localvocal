...
private:
	...
	std::string transcribeWithAmazonTranscribeREST(const float *audio_data, size_t frames, uint32_t sample_rate);
	std::string transcribeWithOpenAI(const float *audio_data, size_t frames, uint32_t sample_rate);
	std::string transcribeWithGoogle(const float *audio_data, size_t frames, uint32_t sample_rate);
	std::string transcribeWithAzure(const float *audio_data, size_t frames, uint32_t sample_rate);
	std::string transcribeWithCustom(const float *audio_data, size_t frames, uint32_t sample_rate);
	
	// Utility functions
	std::string convertAudioToBase64(const float *audio_data, size_t frames, uint32_t sample_rate);
	std::string sendHttpRequest(const std::string &url, const std::string &payload, const std::string &auth_header);
	std::string sendHttpRequestWithHeaders(const std::string &url, const std::string &payload, const std::vector<std::string> &headers);

...
class ForceEventStreamSigV4EndpointProvider final
	: public Aws::TranscribeStreamingService::Endpoint::
		  TranscribeStreamingServiceEndpointProvider {
public:
	Aws::Endpoint::ResolveEndpointOutcome
	ResolveEndpoint(const Aws::Endpoint::EndpointParameters &endpointParameters) const override
	{
		auto outcome = Aws::TranscribeStreamingService::Endpoint::
			TranscribeStreamingServiceEndpointProvider::ResolveEndpoint(
				endpointParameters);
...
{
#if defined(ENABLE_AWS_TRANSCRIBE_SDK)
	if (!initialized_ || !amazon_ ||
	    config_.provider != CloudSpeechProvider::AMAZON_TRANSCRIBE) {
		return;
	}
...

	{
		...
		if (amazon_->audio_samples.size() > kMaxBufferedAudioSamples) {
			const size_t to_drop =
				amazon_->audio_samples.size() - kMaxBufferedAudioSamples;
...
void CloudSpeechProcessor::amazonStreamThreadMain()
{
	...
	if (!is_aws_sdk_initialized()) {
		blog(LOG_ERROR,
		     "[Transcribe] AWS SDK not initialized; cannot start streaming session.");
		return;
	}
	if (config_.region.empty()) {
		blog(LOG_ERROR,
		     "[Transcribe] AWS region is empty; cannot start streaming session.");
		return;
	}
	...
	try {
		...
		if (!config_.session_token.empty()) {
			...
			credentials =
				Aws::Auth::AWSCredentials(config_.api_key, config_.secret_key);
		}

		auto credProvider = Aws::MakeShared<Aws::Auth::SimpleAWSCredentialsProvider>(
			"obs-localvocal", credentials);
		...
		auto build_alternative_text =
			...
			auto build_from_items = [&](bool include_stable_only) -> std::string {
				...
				const bool has_stability_data = std::any_of(
					items.begin(), items.end(),
					[](const auto &item) { return item.StableHasBeenSet(); });
				...
				for (const auto &item : items) {
					...
					if (item.TypeHasBeenSet() &&
					    item.GetType() ==
						    Aws::TranscribeStreamingService::Model::
		...
		handler.SetTranscriptEventCallback(
			[this, build_alternative_text](
				const Aws::TranscribeStreamingService::Model::TranscriptEvent &ev) {
				if (!amazon_) {
					return;
				}
				...
				for (const auto &result : results) {
					...
					if (!amazon_->transcript_updates.empty()) {
						const auto &last =
							amazon_->transcript_updates.back();
						if (last.text == text &&
					...
					if (!is_final) {
						if (!amazon_->transcript_updates.empty() &&
							...
							amazon_->transcript_updates.back().text =
								text;
						} else {
							amazon_->transcript_updates.push_back(
								{text, false});
						}
					} else {
						if (!amazon_->transcript_updates.empty() &&
					...
					while (amazon_->transcript_updates.size() >
					       kMaxQueuedTranscriptUpdates) {
						amazon_->transcript_updates.pop_front();
					}
				}
			});
		...
		handler.SetOnErrorCallback(
			[](const Aws::Client::AWSError<
				Aws::TranscribeStreamingService::TranscribeStreamingServiceErrors>
				   &error) {
				blog(LOG_ERROR, "[Transcribe] Streaming error: %s",
		...
		if (config_.language == "en") {
			...
			request.SetLanguageCode(
				Aws::TranscribeStreamingService::Model::LanguageCode::es_ES);
		} else {
			request.SetLanguageCode(
				Aws::TranscribeStreamingService::Model::LanguageCode::en_US);
		}
		...
		auto onStreamReady = [this](Aws::TranscribeStreamingService::Model::AudioStream
						    &stream) {
			if (!amazon_) {
				return;
			}
			...
			const size_t chunk_samples =
				(kTranscribeSampleRateHz * 20) / 1000; // 20ms chunks
			...
			while (true) {
				std::vector<int16_t> chunk;
				{
					...
					amazon_->cv.wait(lock, [&]() {
						return amazon_->stop_requested ||
						       !amazon_->audio_samples.empty();
					});
					...
					const size_t n = (std::min)(chunk_samples,
								    amazon_->audio_samples.size());
					chunk.reserve(n);
				...
				if (!chunk.empty()) {
					...
					Aws::Vector<unsigned char> bytes(
						p, p + chunk.size() * sizeof(int16_t));
					Aws::TranscribeStreamingService::Model::AudioEvent ev(
						std::move(bytes));
					...
					const int ms = static_cast<int>((chunk.size() * 1000) /
									kTranscribeSampleRateHz);
...
std::string CloudSpeechProcessor::processAudio(const float *audio_data, size_t frames,
...
std::string CloudSpeechProcessor::transcribeWithAmazonTranscribeREST(const float *audio_data,
								     size_t frames,
...
std::string CloudSpeechProcessor::transcribeWithAmazonTranscribe(const float *audio_data,
								 size_t frames,
								 uint32_t sample_rate,
								 bool *out_is_final)
{
	...
	try {
		...
		if (config_.region.empty()) {
			blog(LOG_ERROR,
			     "AWS region is empty; set Cloud Speech -> Region (e.g. us-east-1).");
			return "";
		}
		...
		auto join_transcript = [](const std::string &prefix,
			...
			[&](const Aws::TranscribeStreamingService::Model::TranscriptEvent &ev) {
				...
				for (const auto &result : results) {
					...
					std::string transcriptText =
						build_alternative_text(alternative, is_partial);
					if (transcriptText.empty())
						continue;

					if (is_partial) {
						latest_partial_transcription = join_transcript(
							committed_transcription, transcriptText);
						blog(LOG_INFO, "[partial] %s",
						     latest_partial_transcription.c_str());
					} else {
						std::string resultId;
						...
						if (resultId.empty() ||
						    committed_result_ids.insert(resultId).second) {
							committed_transcription = join_transcript(
								committed_transcription,
								transcriptText);
						}

						latest_final_transcription =
							committed_transcription;
						latest_partial_transcription.clear();
						blog(LOG_INFO, "[final] %s",
						     latest_final_transcription.c_str());
					}
				}
			});
		...
		handler.SetInitialResponseCallbackEx(
			[](const Aws::TranscribeStreamingService::Model::
				   StartStreamTranscriptionInitialResponse &initial,
			   const Aws::Utils::Event::InitialResponseType initialType) {
				blog(LOG_INFO, "[Transcribe] Initial response type: %d",
				     static_cast<int>(initialType));
				if (initial.RequestIdHasBeenSet()) {
					blog(LOG_INFO, "[Transcribe] RequestId: %s",
					     initial.GetRequestId().c_str());
				}
				if (initial.SessionIdHasBeenSet()) {
					blog(LOG_INFO, "[Transcribe] SessionId: %s",
					     initial.GetSessionId().c_str());
				}
			});
		...
		handler.SetOnErrorCallback(
			[](const Aws::Client::AWSError<
				Aws::TranscribeStreamingService::TranscribeStreamingServiceErrors>
				   &error) {
				blog(LOG_ERROR, "AWS Transcribe error: %s",
				     error.GetMessage().c_str());
				blog(LOG_ERROR, "[Transcribe] Exception: %s",
				     error.GetExceptionName().c_str());
				blog(LOG_ERROR, "[Transcribe] HTTP response code: %d",
				     static_cast<int>(error.GetResponseCode()));
				if (!error.GetRequestId().empty()) {
					blog(LOG_ERROR, "[Transcribe] RequestId: %s",
					     error.GetRequestId().c_str());
				}
				if (!error.GetRemoteHostIpAddress().empty()) {
					blog(LOG_ERROR, "[Transcribe] RemoteHostIp: %s",
					     error.GetRemoteHostIpAddress().c_str());
				}
				const auto &headers = error.GetResponseHeaders();
				if (!headers.empty()) {
					blog(LOG_ERROR, "[Transcribe] Response headers (%zu):",
					     headers.size());
					for (const auto &kv : headers) {
						blog(LOG_ERROR, "[Transcribe]   %s: %s",
		...
		Aws::TranscribeStreamingService::Model::StartStreamTranscriptionRequest request;
		request.SetHeadersReceivedEventHandler([](const Aws::Http::HttpRequest *,
							  Aws::Http::HttpResponse *response) {
			if (!response) {
				blog(LOG_WARNING,
				     "[Transcribe] HeadersReceivedEventHandler called with null response");
				return;
			}
			blog(LOG_INFO, "[Transcribe] HTTP headers received, status=%d",
			     static_cast<int>(response->GetResponseCode()));
			const auto &headers = response->GetHeaders();
			if (!headers.empty()) {
				blog(LOG_INFO,
				     "[Transcribe] HTTP response headers (%zu):", headers.size());
				for (const auto &kv : headers) {
					blog(LOG_INFO, "[Transcribe]   %s: %s", kv.first.c_str(),
					     kv.second.c_str());
				}
			}
		});
		...
		if (config_.language == "en") {
			...
			request.SetLanguageCode(Aws::TranscribeStreamingService::Model::
		...
		auto OnStreamReady = [&](Aws::TranscribeStreamingService::Model::AudioStream
						 &stream) {
			...
			if (!stream.WriteAudioEvent(empty_event)) {
				blog(LOG_WARNING,
				     "Failed to write empty audio event to signal end of stream.");
			...
			if (!stream.WaitForDrain(10000)) {
				blog(LOG_WARNING,
				     "Timed out waiting for Transcribe stream drain before Close(); ending stream anyway.");
			}
		...
		if (!committed_transcription.empty()) {
			...
			blog(LOG_INFO, "Returning transcription: %s",
			     committed_transcription.c_str());
...
std::string CloudSpeechProcessor::transcribeWithOpenAI(const float *audio_data, size_t frames,
						       uint32_t sample_rate)
{
	try {
		// Convert audio to base64
		...
		nlohmann::json payload = {{"model", config_.model},
					  {"file", {{"data", audio_base64}, {"mime", "audio/wav"}}},
...
std::string CloudSpeechProcessor::transcribeWithGoogle(const float *audio_data, size_t frames,
						       uint32_t sample_rate)
{
	try {
		// Convert audio to base64
		...
		std::string url =
			"https://speech.googleapis.com/v1/speech:recognize?key=" + config_.api_key;
...
std::string CloudSpeechProcessor::transcribeWithAzure(const float *audio_data, size_t frames,
						      uint32_t sample_rate)
{
	try {
		// Convert audio to base64
		...
		nlohmann::json payload = {{"provider", "Azure"},
					  {"model", config_.model},
					  {"audio",
					   {{"data", audio_base64}, {"mime", "audio/wav"}}},
					  {"language", config_.language}};

		std::string url = "https://" + config_.region +
...
std::string CloudSpeechProcessor::transcribeWithCustom(const float *audio_data, size_t frames,
						       uint32_t sample_rate)
{
	try {
		// Convert audio to base64
		...
		nlohmann::json payload = {{"audio", audio_base64},
...
// Utility functions
std::string CloudSpeechProcessor::convertAudioToBase64(const float *audio_data, size_t frames,
						       uint32_t sample_rate)
{
	try {
		...
		wav_buffer.insert(wav_buffer.end(), reinterpret_cast<const uint8_t *>(&channels),
				  reinterpret_cast<const uint8_t *>(&channels) + 2);
		wav_buffer.insert(wav_buffer.end(), reinterpret_cast<const uint8_t *>(&sample_rate),
				  reinterpret_cast<const uint8_t *>(&sample_rate) + 4);
		wav_buffer.insert(wav_buffer.end(), reinterpret_cast<const uint8_t *>(&byte_rate),
				  reinterpret_cast<const uint8_t *>(&byte_rate) + 4);
		wav_buffer.insert(wav_buffer.end(), reinterpret_cast<const uint8_t *>(&block_align),
		...
		for (size_t i = 0; i < frames; i++) {
			...
			wav_buffer.insert(wav_buffer.end(),
					  reinterpret_cast<const uint8_t *>(&pcm_sample),
					  reinterpret_cast<const uint8_t *>(&pcm_sample) + 2);
		}
		...
		for (size_t i = 0; i < wav_buffer.size(); i += 3) {
			uint32_t triple = (static_cast<uint32_t>(wav_buffer[i]) << 16) |
					  ((i + 1 < wav_buffer.size())
						   ? static_cast<uint32_t>(wav_buffer[i + 1]) << 8
						   : 0) |
					  ((i + 2 < wav_buffer.size())
						   ? static_cast<uint32_t>(wav_buffer[i + 2])
						   : 0);
...
std::string
CloudSpeechProcessor::sendHttpRequestWithHeaders(const std::string &url, const std::string &payload,
...
std::string CloudSpeechProcessor::sendHttpRequest(const std::string &url,
	...
	try {
		...
		case CloudSpeechProvider::GOOGLE:
			if (response_json.contains("results") &&
			    !response_json["results"].empty()) {
				auto &result = response_json["results"][0];
				if (result.contains("alternatives") &&
...
bool CloudSpeechProcessor::retryWithBackoff(std::function<std::string()> operation,
					    std::string &result)
{
	for (int attempt = 0; attempt < config_.max_retries; ++attempt) {
		try {
			...
			blog(LOG_WARNING, "Cloud speech attempt %d failed: %s", attempt + 1,
			     e.what());
		}
